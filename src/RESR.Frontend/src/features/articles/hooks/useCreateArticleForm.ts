...
import type { CreateArticleFormValues } from "@/features/articles/types/article.types";
...
export function useCreateArticleForm() {
  const { token } = useAuth();
  const [values, setValues] = useState<CreateArticleFormValues>(initialValues);
  ...
  function updateField<K extends keyof CreateArticleFormValues>(field: K, value: CreateArticleFormValues[K]) {
    setValues((current) => ({
      ...current,
      [field]: value
    }));
